// src/features/checkout/screens/checkout.screen.js
        ...
        <Spacer position="left" size="medium">
          ...
          <List.Section>
            {cart.map(({ item, price }, i) => (
              <List.Item
                key={`item-${i}`}
                title={`${item} - $${(price / 100).toFixed(2)}`}
              />
            ))}
          </List.Section>

          <Text variant="body">Total: ${(sum / 100).toFixed(2)}</Text>
        </Spacer>
        ...
        <Spacer position="top" size="large">
          {name.length > 0 && (
            <CreditCardInput
              ...
              onError={() =>
                navigation.navigate("CheckoutError", {
                  error: "Error processing card information.",
                })
              }
            />
          )}
...
import { 
  ...
  PayButton, 
  ClearButton, 
  PaymentProcessing } from "../../components/checkout.styles";
